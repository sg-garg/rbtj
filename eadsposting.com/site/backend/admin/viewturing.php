...
$title='Bad Turing Clicks';
admin_login();

list($avg)=@mysql_fetch_row(@mysql_query('select avg(bad_turing) from '.mysql_prefix.'last_login where bad_turing>0'));

if ($connum)
{
    if ($connum!=$md5time)
    {
	    echo "Invalid confirmation ID. Turing Counters not reset";
    }
	else { @mysql_query("update ".mysql_prefix."last_login set bad_turing=0");}
}
...
On average, a member clicked on <?php echo number_format($avg);?> wrong turing numbers.<br>

<br>
To reset Turing counters for all accounts<br>
...
<tr>
    ...
    <b>Wrong turing number counter reset</b>
    </td>
</tr>
<tr>
...
if (!empty($_POST['reset_username']))
{
    ...
    if(!empty($email))
    {
        $resetquery = "UPDATE ".mysql_prefix."last_login SET bad_turing=".(int)$_POST['reset_value']." WHERE username = '$username'";
...
<?php
$report = @mysql_query("select ".mysql_prefix."users.username,bad_turing from ".mysql_prefix."last_login,".mysql_prefix."users where ".mysql_prefix."users.username=".mysql_prefix."last_login.username and account_type!='canceled' and account_type!='suspended' and bad_turing>0 order by bad_turing desc");

echo "<br>
        <table cellpadding='2' cellspacing='0' class='centered' border='1'>
        <tr>
        <td><b>Username</b></td>
        <td><b>Total Bad Clicks</b></td>
     </tr>";
while($row=@mysql_fetch_array($report))
{
    if($bgcolor == ' class="row1" ')
        ...
        $bgcolor=' class="row1" ';
    echo "<tr $bgcolor><td><a href=viewuser.php?userid=$row[username] target=_viewuser>$row[username]</a></td><td align=right>$row[bad_turing]</td></tr>";
}
echo "</table>";
footer();
