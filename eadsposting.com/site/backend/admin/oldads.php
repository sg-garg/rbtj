...
$title='Delete Old eMail Ads';
admin_login();

if (!isset($days)){$days=15;}
echo "<form method=post>List ads that have that have not been clicked in <input type=text name=days value=$days> days. <br><input type=submit name=report value=Report></form>";
if ($connum){
echo "<br><br>";
if ($connum!=$md5time){
...
@mysql_query("drop table ".mysql_prefix."paid_clicks_$key");
@mysql_query("delete from ".mysql_prefix."email_ads where emailid='$key'");
echo "Deleted: $key<br>";
}
...
$report=@mysql_query("select emailid,description,time,run_type,run_quantity,clicks from ".mysql_prefix."email_ads where time<'$time' order by time desc");
...
echo "<table border=1 ><tr><td><b>Delete</b></td><td><b>Ad ID</b></td><td><b>Description</b></td><td><b>Type</b></td><td><b>Expires at</b></td><th>Clicks</th><td><b>Last Click</b>";
...
echo "</td></tr><tr $bgcolor><td><input type=checkbox class=checkbox name=\"uid[$row[emailid]]\" value=1 $checked></td><td>$row[emailid]</a></td><td>$row[description]</td><td>$row[run_type]</td><td>$row[run_quantity]</td><td>$row[clicks]</td><td>".mytimeread($row[time]);
